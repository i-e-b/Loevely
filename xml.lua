...
-- Adapted to use Love2d libraries
-----------------------------------------------------------------------------------------
XmlParser = {};
...
function XmlParser:ParseXmlFile(xmlFileName)
  contents, size = love.filesystem.read( xmlFileName )

	if (not contents) then
		return nil, "Could not read file "..xmlFileName
	else
    return self:ParseXmlText(contents),"ok"
	end
end
